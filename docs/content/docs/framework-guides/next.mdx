...
called from the client via `useMutation` or in server functions and other server
code using `fetchAuthMutation`. Authentication is handled automatically using
session cookies.
