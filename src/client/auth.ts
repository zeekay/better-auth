...
import { ActionCtx } from "../component/_generated/server";
import { jwt } from "better-auth/plugins";
import { bearer } from "better-auth/plugins";
import { oidcProvider } from "better-auth/plugins";
import { Id } from "../component/_generated/dataModel";

export const auth = (database: BetterAuthOptions["database"]) =>
  betterAuth({
...
export const database = (ctx: ActionCtx) => (): Adapter =>
  ({
    id: "convex",
    ...
    findOne: async ({ model, where, select }): Promise<any> => {
      ...
      if (
        ...
        return ctx.runQuery(api.auth.getAccountByAccountIdAndProviderId, {
