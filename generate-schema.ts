...
const mergedIndexFields = Object.fromEntries(
  Object.entries(indexFields).map(([key, value]) => {
    const addedIndexes = Object.keys(
      specialFields[key as keyof typeof specialFields] || {}
    ).filter((index) => !value.includes(index));
    return [key, value.concat(addedIndexes)];
  })
);
console.log("mergedIndexFields", JSON.stringify(mergedIndexFields, null, 2));
